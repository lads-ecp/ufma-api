...
class DocentesCollection (Resource):
    ...
    def get(self):
        ...
        if request.headers['accept'] == 'application/xml':
            dados_rdf = list(map(lambda docente: docenteModelRDF(docente.nome, docente.telefone, docente.urlimg, docente.email,
                                                                 docente.descricao, docente.siape, docente.codigo_subunidade), query.order_by(docenteModelDB.nome).all()))
